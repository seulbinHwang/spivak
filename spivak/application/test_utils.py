...
def test(args: SharedArgs) -> None:
    ...
    if args.test_save_spotting_jsons and args.dataset_type in {
        ...
        write_all_prediction_jsons(results_dir, dataset.video_data,
                                   label_maps[Task.SPOTTING], args.frame_rate)


def _predict_and_save(args: SharedArgs, dataset: Dataset,
                      label_maps: Dict[Task,
                                       LabelMap], results_dir: Path) -> None:
    """For all videos of the dataset, run prediction and save the numpy
    arrays with the results"""
    predictor = load_predictor(args, label_maps, dataset.input_shape,
    ...
    for video_datum in dataset.video_data:
        ...
        predictor.predict_video_and_save(video_datum, flexible_nms,
                                         results_base_path)


def _nms_and_save(args: SharedArgs, dataset: Dataset,
                  label_maps: Dict[Task, LabelMap], results_dir: Path) -> None:
    flexible_nms = create_flexible_nms(args, label_maps[Task.SPOTTING])
    for video_datum in dataset.video_data:
        results_base_path = results_dir / video_datum.relative_path
        detections_path = _output_path(results_base_path,
                                       OUTPUT_DETECTION_SCORE)
        video_outputs = {OUTPUT_DETECTION_SCORE: np.load(str(detections_path))}
        DensePredictor.save_predictions(video_outputs, flexible_nms,
                                        results_base_path)


def _evaluate(args: SharedArgs, dataset: Dataset,
              label_maps: Dict[Task, LabelMap], results_dir: Path) -> None:
    spotting_evaluation = _spotting_evaluation(args, dataset,
                                               label_maps.get(Task.SPOTTING),
                                               results_dir)
    if args.segmentation_evaluation_old:
        _segmentation_evaluation_old(args, dataset,
                                     label_maps.get(Task.SEGMENTATION),
                                     results_dir)
...
def _spotting_evaluation(args: SharedArgs, dataset: Dataset,
    ...
    spotting_evaluation = run_spotting_evaluation(
        ...
        tolerances_config,
        args.frame_rate,
        dataset.num_classes_from_task[Task.SPOTTING],
        prune_classes,
        create_confusion_data_frame=create_confusion_data_frame,
        label_map=label_map)
    logging.info(f"Average-mAP: {spotting_evaluation.average_map_dict}")
...
def _segmentation_evaluation(
    ...
    logging.info(f"Segmentation mean IOU: {segmentation_evaluation.mean_iou}")
...
def _segmentation_evaluation_old(
    ...
    segmentations = [
        np.load(str(_segmentation_path(results_dir /
                                       video_datum.relative_path)))
        for video_datum in dataset.video_data
    ]
    ...
    logging.info(f"Segmentation mean IOU (old version): "
...
def _segmentation_results_available(results_dir: Path,
                                    video_data: List[VideoDatum]) -> bool:
    segmentation_path = _segmentation_path(results_dir /
                                           video_data[0].relative_path)
    return segmentation_path.exists()


def _read_segmentations_and_labels(results_dir: Path,
...
def _spotting_results_available(results_dir: Path,
                                video_data: List[VideoDatum]) -> bool:
    spotting_path = _spotting_path(results_dir / video_data[0].relative_path)
    return spotting_path.exists()


def _read_spotting_detections_and_labels(results_dir: Path,
                                         video_data: List[VideoDatum]):
    detections = []
    labels = []
    for video_datum in video_data:
        ...
        if labels_path.exists():
            labels.append(np.load(str(labels_path)))
            detections_path = _spotting_path(results_dir /
                                             video_datum.relative_path)
