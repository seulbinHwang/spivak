...
class VideoInfo:
    ...
    def __init__(self, video_path: Path, features_dir: Path,
...
class FeatureExtractor:
    ...
    def __init__(self, raw_feature_extractor: RawFeatureExtractorInterface,
                 feature_file_prefix: str,
                 pca_transformer: PCAInterface) -> None:
...
def extract_features_from_videos(video_paths: List[Path], features_dir: Path,
...
def create_feature_extractor(extractor_type: str,
                             model_dir: Path) -> FeatureExtractor:
    if extractor_type == EXTRACTOR_TYPE_RESNET_TF2:
        ...
        soccernet_feature_extractor = FeatureExtractorResNetTF2(resnet_tf2_path,
                                                                fps=fps)
    ...
    return FeatureExtractor(soccernet_feature_extractor, feature_file_prefix,
                            pca_transformer)
...
def _create_video_infos(video_paths: List[Path],
                        features_dir: Path) -> List[VideoInfo]:
    return [
        _create_video_info(video_path, features_dir)
...
def _make_features(video_infos: List[VideoInfo],
                   feature_extractor: FeatureExtractor) -> None:
    for video_info in video_infos:
