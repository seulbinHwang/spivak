...
class BottomUpStackInterface(metaclass=ABCMeta):
    ...
    @abstractmethod
    def downsample_and_convolve(self, bottom_up: Tensor, num_filters_in: int,
                                num_filters_out: int,
                                layer_index: int) -> Tensor:
        pass

    @abstractmethod
    def convolve(self, bottom_up: Tensor, num_filters: int,
                 layer_index: int) -> Tensor:
        pass
...
class PoolingBottomUpStack(BottomUpStackInterface):
    ...
    def convolve(self, bottom_up: Tensor, num_filters: int,
                 layer_index: int) -> Tensor:
        return self._convolution_stack.convolve(bottom_up, num_filters,
    ...
    def downsample_and_convolve(self, bottom_up: Tensor, num_filters_in: int,
        ...
        return self._convolution_stack.convolve(pooled, num_filters_out,
                                                layer_index, self._name)


class StridedBottomUpStack(BottomUpStackInterface):

    def __init__(self, strided_block: StridedBlockInterface,
...
def create_bottom_up_layers(
        ...
        max_num_filters: int,
        bottom_up_stack: BottomUpStackInterface) -> List[BottomUpLayer]:
    ...
    for layer_index in range(1, num_layers):
        ...
        num_filters_out = min(max_num_filters,
                              2**layer_index * base_num_filters)
        x = bottom_up_stack.downsample_and_convolve(x, num_filters_in,
                                                    num_filters_out,
                                                    layer_index)
        bottom_up_layers.append(BottomUpLayer(x, num_filters_out))
