...
class TrainerHeadInterface(metaclass=ABCMeta):
    ...
    @abstractmethod
    def chunk_targets(self, video_targets_and_weights, start: int,
...
def create_head_stack(name: str, weight_decay: float, batch_norm: int,
...
def get_chunk_targets_and_weights(video_targets_and_weights, start: int,
                                  mask: np.ndarray, num_chunk_frames: int,
                                  weight_creator: WeightCreatorInterface,
...
def get_multidimensional_chunk_targets_and_weights(
    ...
    weight_indices = range(num_targets_and_weights // 2,
                           num_targets_and_weights)
    if valid:
        for weight_index, weight_creator in zip(weight_indices,
                                                weight_creators):
...
def get_tf_targets_and_weights_mapper(video_targets_and_weight_inputs,
...
def get_tf_multidimensional_targets_and_weights_mapper(
    ...
    def create_chunk_targets_and_weights(chunk_start_and_mask):
        ...
        chunk_weight_inputs = chunk_targets_and_weight_inputs[:, :, num_targets:
                                                              num_targets_and_weights]
        ...
        chunk_weights_unmasked = tf.transpose(chunk_weights_array.stack(),
                                              perm=[1, 2, 0])
        mask_weights = tf.cast(tf.logical_not(chunk_mask), tf.float32)
        expanded_mask_weights = tf.expand_dims(tf.expand_dims(mask_weights, -1),
                                               -1)
        tiled_mask_weights = tf.tile(expanded_mask_weights,
                                     (1, num_classes, num_weights))
        chunk_weights = tf.multiply(chunk_weights_unmasked, tiled_mask_weights)
        if not valid:
            chunk_weights = 0.0 * chunk_weights
        chunk_targets_and_weights = tf.concat([chunk_targets, chunk_weights],
                                              axis=2)
        return tf_pad_frames_on_right_3d(chunk_targets_and_weights,
                                         num_chunk_frames)
...
def tf_pad_frames_on_right_2d(sliced_tensor: Tensor,
                              num_chunk_frames) -> Tensor:
    paddings = [[0, num_chunk_frames - tf.shape(sliced_tensor)[0]], [0, 0]]
    return tf.pad(sliced_tensor, paddings, mode='CONSTANT', constant_values=0)


def tf_pad_frames_on_right_3d(sliced_tensor: Tensor,
                              num_chunk_frames) -> Tensor:
    paddings = [[0, num_chunk_frames - tf.shape(sliced_tensor)[0]], [0, 0],
                [0, 0]]
    return tf.pad(sliced_tensor, paddings, mode='CONSTANT', constant_values=0)
