...
class ConvolutionStackInterface(metaclass=ABCMeta):
    ...
    @abstractmethod
    def convolve(self, tensor: Tensor, num_filters: int, layer_index: int,
                 name: str) -> Tensor:
        pass
...
class StridedBlockInterface(ConvolutionBlockInterface):
    ...
    @abstractmethod
    def strided_convolve(self, tensor: Tensor, num_filters: int,
                         name: str) -> Tensor:
        pass
...
class BasicConvolutionStack(ConvolutionStackInterface):
    ...
    def __init__(self, convolution_block: ConvolutionBlockInterface,
...
class BasicConvolutionBlock(ConvolutionBlockInterface):
    ...
    def __init__(self, padding: str, weight_decay: float, batch_norm: int,
    ...
    def convolve(self, tensor: Tensor, num_filters: int, name: str) -> Tensor:
        return convolution_wrapper(tensor,
                                   num_filters, (3, 1), (1, 1),
                                   self._padding,
                                   self._weight_decay,
                                   self._batch_norm,
...
class ResidualBlockV2(StridedBlockInterface):
    ...
    def __init__(self, num_convolutions: int, skip_init: bool, bottleneck: bool,
                 weight_decay: float, batch_norm: int,
    ...
    def _residual_block(self, input_tensor: Tensor, num_filters: int,
                        final_strides: Tuple[int, int], name: str) -> Tensor:
        input_tensor_relu = Activation("relu",
                                       name=f"{name}_conv0_relu")(input_tensor)
        tensor = self._internal_convolutions(input_tensor_relu, num_filters,
                                             final_strides, name)
        if self._should_use_shortcut():
            shortcut = self._prepare_shortcut(input_tensor, input_tensor_relu,
                                              num_filters, final_strides, name)
            if self._skip_init:
    ...
    def _internal_convolutions(self, tensor: Tensor, num_filters: int,
                               final_strides: Tuple[int,
                                                    int], name: str) -> Tensor:
        if self._bottleneck:
            return self._bottleneck_convolutions(tensor, num_filters,
                                                 final_strides, name)
        else:
            return self._standard_convolutions(tensor, num_filters,
                                               final_strides, name)
    ...
    def _prepare_shortcut(self, input_tensor: Tensor, input_tensor_relu: Tensor,
                          num_filters: int, strides: Tuple[int, int],
                          name: str) -> Tensor:
        if num_filters != input_tensor.shape[3]:
            ...
            return convolution_wrapper(input_tensor_relu,
                                       num_filters, (1, 1),
                                       strides,
                                       "same",
                                       self._weight_decay,
    ...
    def _standard_convolutions(self, input_tensor_relu: Tensor,
                               num_filters: int, final_strides: Tuple[int, int],
                               name: str) -> Tensor:
        ...
        for convolution_index in range(0, self._num_convolutions):
            ...
            if convolution_index == self._num_convolutions - 1:
                ...
                strides = (1, 1)
            tensor = convolution_wrapper(tensor,
                                         num_filters, (3, 1),
                                         strides,
                                         "same",
                                         self._weight_decay,
                                         ...
                                         self._dropout_rate,
                                         INITIALIZER_FOR_RELU,
                                         name=f"{name}_conv{convolution_index}")
        return tensor

    def _bottleneck_convolutions(self, input_tensor_relu: Tensor,
                                 num_filters: int, final_strides: Tuple[int,
                                                                        int],
                                 name: str) -> Tensor:
        ...
        tensor = convolution_wrapper(input_tensor_relu,
                                     bottleneck_num_filters, (1, 1), (1, 1),
                                     "valid",
                                     self._weight_decay,
                                     self._batch_norm,
                                     self._dropout_rate,
        ...
        tensor = ZeroPadding2D(padding=((1, 1), (0, 0)),
                               name=f"{name}_pad")(tensor)
...
class ResidualBlock(StridedBlockInterface):
    ...
    def __init__(self, num_convolutions: int, bottleneck: bool,
                 weight_decay: float, batch_norm: int,
                 dropout_rate: float) -> None:
        if bottleneck:
            assert num_convolutions == 1
    ...
    def _residual_block(self, input_tensor: Tensor, num_filters: int,
        ...
        tensor = self._internal_convolutions(input_tensor, num_filters,
                                             initial_strides, name)
        if self._should_use_shortcut():
            shortcut = self._prepare_shortcut(input_tensor, num_filters,
                                              initial_strides, name)
            addition = Add(name=f"{name}_shortcut_add")
            tensor = addition([shortcut, tensor])
    ...
    def _internal_convolutions(self, tensor: Tensor, num_filters: int,
        ...
        if self._bottleneck:
            return self._bottleneck_convolutions(tensor, num_filters, strides,
                                                 name)
        else:
            return self._standard_convolutions(tensor, num_filters, strides,
                                               name)
    ...
    def _prepare_shortcut(self, tensor: Tensor, num_filters: int,
                          strides: Tuple[int, int], name: str) -> Tensor:
    ...
    def _standard_convolutions(self, tensor: Tensor, num_filters: int,
    ...
    def _bottleneck_convolutions(self, tensor: Tensor, num_filters: int,
                                 strides: Tuple[int, int], name: str) -> Tensor:
        assert num_filters % 4 == 0
        bottleneck_num_filters = num_filters // 4
        ...
        tensor = convolution_wrapper(tensor,
                                     bottleneck_num_filters, (3, 1), (1, 1),
                                     "same",
                                     self._weight_decay,
                                     self._batch_norm,
                                     self._dropout_rate,
