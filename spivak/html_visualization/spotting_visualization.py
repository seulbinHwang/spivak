...
def add_detection_scores_graphs(html_file: TextIO, data_frame: DataFrame,
    ...
    for category_index, category in enumerate(
        ...
        _fig_add_detection_scores(fig, category_data_frame, category,
                                  category_color, row)
    ...
    fig.update_layout(title_text=DETECTION_SCORES_TITLE,
                      height=DETECTION_SCORES_HEIGHT)
    fig.write_html(html_file)


def add_predictions_graphs(html_file: TextIO, data_frame: DataFrame,
                           columns: List[str],
                           category_settings: CategorySettings) -> None:
    n_categories = len(category_settings.discrete_color_map)
    ...
    for category_index, category in enumerate(category_order):
        ...
        category_data_frame = data_frame[data_frame[COLUMN_CLASS] == category]
        _fig_add_predictions(fig, category_data_frame, columns,
                             column_color_map, show_legend, row)
        _fig_add_detections_grouped(fig, category_data_frame, COLUMN_DETECTION,
                                    detection_color, show_legend, row)
        if COLUMN_SPOTTING_LABEL in category_data_frame.columns:
            label_color = column_color_map[COLUMN_SPOTTING_LABEL]
            _fig_add_labels_grouped(fig, category_data_frame,
                                    COLUMN_SPOTTING_LABEL, label_color,
                                    show_legend, row)
    ...
    fig.update_annotations(textangle=-90,
                           ...
                           font=dict(size=12))
    adjust_subplot_xaxes(fig, n_categories)
    fig.update_layout(title_text=PREDICTIONS_TITLE, height=PREDICTIONS_HEIGHT)
    fig.write_html(html_file)


def add_detections_graph(html_file: TextIO, data_frame: DataFrame,
                         category_settings: CategorySettings) -> None:
    fig = _plot_all_detections_and_labels(data_frame, category_settings,
                                          DETECTIONS_HEIGHT)
...
def _fig_add_predictions(fig: Figure, category_data_frame: DataFrame,
                         columns: List[str], column_color_map: Dict[str, str],
                         show_legend: bool, row: int) -> None:
    for column in columns:
        if column == COLUMN_DELTA:
            marker_dict = dict(color=column_color_map[column], size=2)
            fig.add_scatter(x=category_data_frame[COLUMN_TIME],
                            y=category_data_frame[column],
                            name=column,
                            legendgroup=column,
                            mode="markers",
                            marker=marker_dict,
            ...
            fig.add_scatter(x=filtered_data_frame[COLUMN_TIME],
                            y=filtered_data_frame[column],
                            name=column,
                            legendgroup=column,
                            mode="markers",
                            marker=marker_dict,
...
def _fig_add_labels_grouped(fig: Figure, category_data_frame: DataFrame,
                            name: str, color: str, show_legend: bool,
                            row: int) -> None:
    x, y = extract_locations(category_data_frame, COLUMN_SPOTTING_LABEL,
...
def _fig_add_detections_grouped(fig: Figure, category_data_frame: DataFrame,
                                name: str, color: str, show_legend: bool,
                                row: int) -> None:
    x, y = extract_locations(category_data_frame, COLUMN_DETECTION,
                             PREDICTIONS_Y_DETECTIONS)
...
def _fig_add_detections(fig: Figure, category_data_frame: DataFrame, name: str,
                        color: str, row: int) -> None:
    x, y = extract_locations(category_data_frame, COLUMN_DETECTION,
                             PREDICTIONS_Y_DETECTIONS)
    detection_marker = _create_detection_marker(color)
...
def _plot_all_detections_and_labels(data_frame: DataFrame,
                                    category_settings: CategorySettings,
                                    height: int) -> Figure:
    # Get detections from data frame
    detections_and_labels_data_frame = data_frame[data_frame[COLUMN_DETECTION]
                                                  == 1].copy()
    ...
    if COLUMN_SPOTTING_LABEL in data_frame.columns:
        labels_data_frame = data_frame[data_frame[COLUMN_SPOTTING_LABEL] ==
                                       1].copy()
    ...
    fig = px.scatter(detections_and_labels_data_frame,
                     x=COLUMN_TIME,
