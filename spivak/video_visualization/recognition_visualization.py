...
def get_label(score: float, recognition_threshold: float,
              class_name: str) -> Label:
    return Label(create_label_text(score, class_name), score
                 > recognition_threshold)


def get_multi_labels(frames_actions_view: FrameRecognizedActionsView,
                     recognition_threshold: float) -> List[Label]:
    return [
        get_label(score, recognition_threshold,
...
def get_bold_font_thickness(standard_font_thickness: int,
                            font_scale: float) -> int:
...
def get_text_size(label: Label, font_scale: float, standard_font_thickness: int,
...
def cv2_draw_labels(np_image: np.ndarray, frame_time: float,
    ...
    bold_font_thickness = get_bold_font_thickness(standard_font_thickness,
                                                  font_scale)
    text_sizes = [
    ...
    location_y = math.ceil(image_height - max_baseline -
                           font_scale * BOTTOM_LABEL_TEXT_RELATIVE_GAP_Y)
    ...
    for i, (label, location_x) in enumerate(zip(labels, location_xs)):
        ...
        cv2.putText(np_image, text, (location_x, location_y), FONT, font_scale,
                    CV_COLOR_TEXT, font_thickness)


def compute_labels_xs(font_scale: float,
                      text_sizes: List[OpenCVTextSizes]) -> List[int]:
    label_widths = np.array([t[0][0] for t in text_sizes])
    relative_shifts = np.insert(label_widths[:-1], 0, 0)
    relative_shifts_with_gaps = (relative_shifts +
                                 font_scale * LABEL_TEXT_RELATIVE_GAP_X)
...
def _limit_font_scale(target_font_scale: float, image_height: int) -> float:
    min_font_scale = max(FONT_SCALE_MIN_RELATIVE * image_height,
                         FONT_SCALE_MIN_ABSOLUTE)
