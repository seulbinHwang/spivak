...
DrawActions = Callable[[np.ndarray, TransformationSizes, float], np.ndarray]
ApplyFilters = Callable[[VideoFrame], VideoFrame]


def create_video_from_containers(in_container: InputContainer,
                                 out_container: OutputContainer,
                                 draw_on_av_frame: DrawOnAvFrame,
                                 add_border: bool) -> None:
    ...
    out_video_stream = _set_up_out_video_stream(out_container,
                                                transformation_sizes,
                                                in_video_stream)
    ...
    apply_filters = _create_filters(transformation_sizes.resizing_image_size,
                                    in_video_stream)
...
def _compute_transformation_sizes(in_video_stream: VideoStream,
                                  add_border: bool) -> TransformationSizes:
    ...
    if not add_border:
        ...
        canvas_image_size = ImageSize(resized_height + extra_canvas_height,
                                      resized_width)
...
def _set_up_out_video_stream(out_container: OutputContainer,
...
def create_silent_frame() -> AudioFrame:
    silence = np.zeros(OUT_SILENT_AUDIO_NUMPY_SHAPE,
                       dtype=OUT_SILENT_AUDIO_NUMPY_TYPE)
    audio_frame = AudioFrame.from_ndarray(silence,
                                          format=OUT_SILENT_AUDIO_FORMAT,
                                          layout=OUT_SILENT_AUDIO_LAYOUT)
...
def add_video_frame(out_frame: VideoFrame, out_video_stream: VideoStream,
                    out_container: OutputContainer) -> None:
...
def apply_draw_to_av_frame(in_frame: VideoFrame,
                           transformation_sizes: TransformationSizes,
...
def add_canvas_border(np_image: np.ndarray,
...
def _create_filters(resizing_image_size: Optional[ImageSize],
