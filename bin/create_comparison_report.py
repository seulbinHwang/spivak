...
PDF_TOLERANCE_GRAPH_SETTINGS = [
    ...
    (TOLERANCE_MEDIUM_GRAPH_TITLE, TOLERANCE_MEDIUM_GRAPH_PDF_FILE, 20.0, True,
     TOLERANCE_MEDIUM_GRAPH_PDF_HEIGHT, TOLERANCE_MEDIUM_GRAPH_PDF_WIDTH),
    (TOLERANCE_SMALL_GRAPH_TITLE, TOLERANCE_SMALL_GRAPH_PDF_FILE, 10.0, True,
     TOLERANCE_GRAPH_PDF_HEIGHT, TOLERANCE_GRAPH_PDF_WIDTH)
]
# AP per class graph
...
def main() -> None:
    ...
    with html_path.open("w") as html_file:
        ...
        _write_selected_classes(html_file,
                                first_spotting_evaluation.selected_classes,
                                first_spotting_evaluation.label_map)
        _write_tolerances(html_file,
                          first_spotting_evaluation.tolerances_in_seconds_dict)
        average_map_fig = _create_average_map_graph(evaluation_runs, methods)
        average_map_fig.write_html(html_file)
        class_fig = _create_class_graph(evaluation_runs, methods,
                                        ordered_class_names)
        ...
        tolerance_fig = _create_tolerance_graph(evaluation_runs,
                                                methods,
                                                None,
                                                TOLERANCE_GRAPH_HEIGHT,
                                                TOLERANCE_GRAPH_WIDTH,
                                                True,
                                                TOLERANCE_GRAPH_TITLE,
                                                pdf=False)
        ...
        if CREATE_PAPER_PDFS:
            for (title, file_name, tolerance_limit, show_legend, height,
                ...
                tolerance_pdf_fig = _create_tolerance_graph(evaluation_runs,
                                                            methods,
                                                            tolerance_limit,
                                                            height,
                                                            width,
        ...
        ap_per_class_fig = _create_ap_per_class_graph(ap_data_frame, methods,
                                                      ordered_class_names)
        ...
        if first_spotting_evaluation.confusion_data_frame is not None:
            ...
            for pr_class_set in pr_selected_class_sets:
                ...
                _write_selected_classes(html_file, pr_selected_classes,
                                        label_map)
...
def _write_selected_classes(html_file: TextIO, selected_classes: List[bool],
...
def _get_command_line_arguments() -> Dict:
    ...
    parser.add_argument("--" + Args.OUT_DIR,
...
def _create_ap_data_frame(evaluation_runs: List[EvaluationRun],
                          methods: List[str]) -> DataFrame:
    data_frames = []
...
def _create_pr_data_frame(evaluation_runs: List[EvaluationRun],
    ...
    drop_selection = ((concatenated[SpottingEvaluation.PRECISION] == 0) &
...
def _create_pr_per_tolerance_data_frame(
    ...
    # Sort the precision and recall values according to their respective
    # thresholds so that it is easier to plot them later.
    pr_per_tolerance_data_frame.sort_values(SpottingEvaluation.THRESHOLD,
                                            inplace=True)
...
def _create_method_pr_per_tolerance_data_frame(
    ...
    aggregate_data_frame.drop(aggregate_data_frame[drop_selection].index,
                              inplace=True)
...
def _add_class_name_mapping(data_frame: DataFrame,
                            label_map: LabelMap) -> DataFrame:
    data_frame[SpottingEvaluation.CLASS_NAME] = data_frame[
        SpottingEvaluation.CLASS_INDEX].map(label_map.int_to_label)
    return data_frame


def _create_average_map_graph(evaluation_runs: List[EvaluationRun],
                              methods: List[str]) -> Figure:
    rows = []
    ...
    tolerances_names = sorted(evaluation_runs[0].evaluation.spotting_evaluation.
                              average_map_dict.keys())
    ...
    fig.update_layout(title_text=AVERAGE_MAP_GRAPH_TITLE,
                      height=AVERAGE_MAP_GRAPH_HEIGHT,
                      width=AVERAGE_MAP_GRAPH_WIDTH)
    return fig


def _create_class_graph(evaluation_runs: List[EvaluationRun],
                        methods: List[str],
    ...
    for evaluation_run, method in zip(evaluation_runs, methods):
        ...
        rows.extend(
            _create_method_class_average_ap_rows(class_average_ap_dict, method,
                                                 label_map))
    ...
    active_tolerances_average_ap_values = [
        _create_method_class_average_ap_values(
            ...
            ordered_class_names) for method in methods
    ]
    fig = px.bar(x=ordered_class_names,
                 y=active_tolerances_average_ap_values,
                 barmode="group")
    ...
    for tolerances_name in tolerances_names:
        ...
        new_button = dict(method="restyle",
                          visible=True,
                          args=args,
    ...
    fig.update_layout(title_text=CLASS_GRAPH_TITLE,
                      xaxis_title=CLASS_AXIS_TITLE,
                      yaxis_title=AVERAGE_AP_AXIS_TITLE,
                      height=CLASS_GRAPH_HEIGHT,
                      width=CLASS_GRAPH_WIDTH,
                      legend_title_text=METHOD,
                      updatemenus=update_menus)
    return fig


def _create_ap_per_tolerance_graph(ap_data_frame: DataFrame, methods: List[str],
                                   ordered_class_names: List[str]) -> Figure:
    ...
    active_tolerance_ap_data_frame = ap_data_frame[ap_data_frame[
        SpottingEvaluation.TOLERANCE] == active_tolerance]
    active_tolerance_ap_values = [
        _create_method_tolerance_ap_values(active_tolerance_ap_data_frame,
                                           method, ordered_class_names)
        for method in methods
    ]
    fig = px.bar(x=ordered_class_names,
                 y=active_tolerance_ap_values,
                 barmode="group")
    _update_trace_names(fig, methods)
    buttons = []
    # button with one option for each dataframe
    for tolerance in tolerances:
        tolerance_ap_data_frame = ap_data_frame[ap_data_frame[
            SpottingEvaluation.TOLERANCE] == tolerance]
        tolerance_ap_values = [
            _create_method_tolerance_ap_values(tolerance_ap_data_frame, method,
                                               ordered_class_names)
            for method in methods
        ]
    ...
    fig.update_layout(title_text=AP_PER_TOLERANCE_GRAPH_TITLE,
                      xaxis_title=CLASS_AXIS_TITLE,
                      yaxis_title=AP_AXIS_TITLE,
                      height=AP_PER_CLASS_TOLERANCE_GRAPH_HEIGHT,
                      width=AP_PER_TOLERANCE_GRAPH_WIDTH,
                      legend_title_text=METHOD,
                      updatemenus=update_menus)
    return fig


def _extract_tolerances_names(data_frame: DataFrame,
...
def _extract_tolerances(data_frame: DataFrame,
                        methods: List[str]) -> List[float]:
    tolerance_set = set(data_frame[SpottingEvaluation.TOLERANCE])
    for method in methods:
        method_tolerance_set = set(data_frame[data_frame[METHOD] == method][
...
def _create_tolerance_graph(evaluation_runs: List[EvaluationRun],
                            methods: List[str],
                            tolerance_limit: Optional[float], height: int,
                            width: int, show_legend: bool, title: str,
                            pdf: bool) -> Figure:
    fig = go.Figure()
    for i, evaluation_run in enumerate(evaluation_runs):
        ...
        fig.add_scatter(x=sorted_tolerances,
                        y=sorted_average_map,
                        mode="markers+lines",
                        name=name,
                        marker=marker_dict)
    if pdf:
        template = "none"
        legend_dict = dict(yanchor="bottom",
                           y=0.07,
                           xanchor="right",
                           x=0.995,
                           borderwidth=1)
    ...
    fig.update_layout(title_text=title,
                      xaxis_title=MATCHING_TOLERANCE_AXIS_TITLE,
                      yaxis_title=MAP_AXIS_TITLE,
                      height=height,
                      width=width,
                      template=template,
                      legend=legend_dict,
...
def _create_ap_per_class_graph(ap_data_frame: DataFrame, methods: List[str],
                               ordered_class_names: List[str]) -> Figure:
    ...
    active_class_data_frame = ap_data_frame[ap_data_frame[
        SpottingEvaluation.CLASS_NAME] == active_class_name]
    ...
    # relevant tolerances.
    active_class_data_frame = active_class_data_frame[active_class_data_frame[
        SpottingEvaluation.TOLERANCE].isin(tolerances)]
    # Finally, we get the relevant ap values from the data frame.
    active_class_ap_values = _all_methods_ap_values(active_class_data_frame,
                                                    methods)
    ...
    fig.update_layout(title_text=AP_PER_CLASS_GRAPH_TITLE,
                      xaxis_title=MATCHING_TOLERANCE_AXIS_TITLE,
                      yaxis_title=AP_AXIS_TITLE,
                      height=AP_PER_CLASS_CLASS_GRAPH_HEIGHT,
                      width=AP_PER_CLASS_GRAPH_WIDTH,
                      legend_title_text="",
                      updatemenus=update_menus)
    return fig
...
def _class_ap_values(ap_data_frame, class_name, methods):
    class_data_frame = ap_data_frame[ap_data_frame[
        SpottingEvaluation.CLASS_NAME] == class_name]
...
def _all_methods_ap_values(ap_data_frame, methods):
    return [
        ap_data_frame[ap_data_frame[METHOD] == method][
            SpottingEvaluation.AVERAGE_PRECISION].values for method in methods
    ]


def _create_method_class_average_ap_rows(class_average_ap_dict: Dict[
    str, List[float]], method: str, label_map: LabelMap) -> List[Dict]:
    return [
        {
...
def _create_method_class_average_ap_values(tolerances_ap_data_frame: DataFrame,
                                           method: str,
    ...
    return [
        method_tolerances_ap_data_frame[method_tolerances_ap_data_frame[
            SpottingEvaluation.CLASS_NAME] == class_name][AVERAGE_AP].values[0]
        for class_name in ordered_class_names
    ]


def _create_method_tolerance_ap_values(tolerance_ap_data_frame, method,
                                       ordered_class_names: List[str]):
    method_tolerance_ap_data_frame = tolerance_ap_data_frame[
        tolerance_ap_data_frame[METHOD] == method]
    return [
        method_tolerance_ap_data_frame[method_tolerance_ap_data_frame[
            SpottingEvaluation.CLASS_NAME] == class_name][
                SpottingEvaluation.AVERAGE_PRECISION].values[0]
        for class_name in ordered_class_names
    ]
...
def _create_pr_per_class_and_tolerance_graph(
    ...
    fig = px.line(active_pr_data_frame,
                  y=SpottingEvaluation.PRECISION,
                  x=SpottingEvaluation.RECALL,
                  text=SpottingEvaluation.THRESHOLD,
                  color=METHOD,
                  category_orders={METHOD: methods})
    buttons = []
    for tolerance in tolerances:
        pr_tolerance_data_frame = pr_data_frame[pr_data_frame[
            SpottingEvaluation.TOLERANCE] == tolerance]
        for class_name in ordered_class_names:
    ...
    fig.update_traces(mode="markers+lines")
    update_menus = [
        dict(buttons=buttons, direction="down", showactive=True, active=0)
    ]
    fig.update_layout(title_text=PR_GRAPH_TITLE,
                      xaxis_title=RECALL_AXIS_TITLE,
                      yaxis_title=PRECISION_AXIS_TITLE,
                      height=PR_GRAPH_HEIGHT,
                      width=PR_GRAPH_WIDTH,
                      legend_title_text="",
                      updatemenus=update_menus,
                      xaxis_range=PR_RECALL_RANGE,
                      yaxis_range=PR_PRECISION_RANGE)
    return fig


def _create_pr_per_tolerance_graph(pr_data_frame: DataFrame,
                                   methods: List[str]) -> Figure:
    tolerances = _extract_tolerances(pr_data_frame, methods)
    active_tolerance = tolerances[0]
    active_pr_data_frame = pr_data_frame[pr_data_frame[
        SpottingEvaluation.TOLERANCE] == active_tolerance]
    # For some reason, I'm not able to directly provide x, y and text as data
    # here, so I have to use provide the DataFrame instead into px.line().
    fig = px.line(active_pr_data_frame,
    ...
    for tolerance in tolerances:
        ...
        args = [{
            "y": tolerance_precisions,
            "x": tolerance_recalls,
            "text": tolerance_thresholds,
            "labels": methods
        }]
    ...
    fig.update_layout(title_text=PR_PER_TOLERANCE_GRAPH_TITLE,
                      xaxis_title=RECALL_AXIS_TITLE,
                      yaxis_title=PRECISION_AXIS_TITLE,
...
def _tolerance_precision_recall(pr_data_frame: DataFrame, tolerance: float,
                                methods: List[str]):
    pr_tolerance_data_frame = pr_data_frame[pr_data_frame[
        SpottingEvaluation.TOLERANCE] == tolerance]
...
def _class_precision_recall(pr_data_frame, class_name, methods):
    class_data_frame = pr_data_frame[pr_data_frame[
        SpottingEvaluation.CLASS_NAME] == class_name]
