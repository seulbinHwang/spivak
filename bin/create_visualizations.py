...
def main() -> None:
    ...
    for video_relative_path in sorted(video_relative_paths):
        ...
        in_video_path = _get_in_video_path(input_videos_dir,
                                           video_relative_path)
        video_dir = visualizations_dir / video_relative_path
        video_dir.mkdir(exist_ok=True, parents=True)
        out_video_path = video_dir / (video_relative_path.name +
                                      OUT_VIDEO_EXTENSION)
        _create_out_video(video, in_video_path, out_video_path,
                          should_create_videos, label_map)
        ...
        if (video.spotting_results is not None or
            ...
            with html_path.open("w") as html_file:
                if video.spotting_results is not None:
                    ...
                    _write_spotting_html(video, out_video_path, video_dir,
                                         label_map, debug_graphs)
                if video.segmentation_results is not None:
                    html_file.write(
                        f'<a href="{SEGMENTATION_HTML}">Segmentation</a><br>\n')
                    _write_segmentation_html(video, out_video_path, video_dir,
                                             label_map, debug_graphs)
...
def _get_command_line_arguments() -> Dict:
    ...
    parser.add_argument("--" + Args.INPUT_VIDEOS_DIR,
                        ...
                        required=True)
    parser.add_argument("--" + Args.VISUALIZATIONS_DIR,
                        help="Output directory",
                        required=True)
    parser.add_argument("--" + Args.RESULTS_DIR,
                        required=True,
                        help="Directory containing the saved result files")
    parser.add_argument("--" + Args.LABEL_MAP,
                        help="CSV file containing label names and ids",
                        required=True)
    parser.add_argument(
        '--' + Args.CREATE_VIDEOS,
        dest=Args.CREATE_VIDEOS,
        help="Create videos with visualizations overlaid on them",
        action='store_true',
        default=False)
    parser.add_argument(
        '--no_' + Args.CREATE_VIDEOS,
        dest=Args.CREATE_VIDEOS,
...
def _get_in_video_path(input_videos_dir: Path,
                       video_relative_path: Path) -> Path:
...
def _max_recognized_action(
    ...
    return FrameRecognizedActions(
        _concat_and_max([
            frame_recognized_actions.scores
            for frame_recognized_actions in frame_recognized_actions_list
        ]),
        _concat_and_max([
            frame_recognized_actions.detections
            for frame_recognized_actions in frame_recognized_actions_list
        ]),
        _concat_and_max([
            frame_recognized_actions.labels
            for frame_recognized_actions in frame_recognized_actions_list
        ]))
...
def _create_out_video(video: Video, in_video_path: Path, out_video_path: Path,
    ...
    if out_video_path.exists():
        ...
        if video.recognized_actions:
            ...
            _run_timed_create_a_video(str(in_video_path), str(out_video_path),
                                      coarse_video_recognized_actions,
                                      label_map)
    else:
...
def _run_timed_create_a_video(
    ...
    def create_a_video() -> None:
        _create_video(in_video_path, out_video_path, video_recognized_actions,
...
def _create_video(in_video_path: str, out_video_path: str,
                  video_recognized_actions: VideoSoccerRecognizedActions,
                  recognition_label_map: LabelMap) -> None:
    import av
    ...
    def draw_actions(np_image: np.ndarray,
                     transformation_sizes: TransformationSizes,
                     frame_time: float) -> np.ndarray:
        np_image = add_canvas_border(np_image,
                                     transformation_sizes.canvas_image_size)
    ...
    def draw_on_av_frame(
        ...
        return apply_draw_to_av_frame(in_frame, transformation_sizes,
                                      draw_actions)

    with av.open(in_video_path) as in_container, \
            av.open(out_video_path, mode='w') as out_container:
        create_video_from_containers(in_container,
                                     out_container,
                                     draw_on_av_frame,
                                     add_border=True)
...
def _get_frame_recognized_actions_view_from_video(
    ...
    time_diff, best_time = min((abs(action_time - frame_time), action_time)
...
def _write_segmentation_html(video: Video, out_video_path: Path,
                             video_dir: Path, label_map: LabelMap,
                             debug_graphs: bool) -> None:
    ...
    with html_path.open("w") as html_file:
        ...
        add_video(html_file, out_video_path.relative_to(video_dir),
                  video.video_id)
        html_file.write('</div>\n')
        html_file.write('<div style="overflow-y:auto;height:74%;border:1px '
                        'solid black;">')
        html_file.write('<table width="99%"><tr><td>\n')
        _add_segmentation_graphs(html_file, video.segmentation_results,
                                 class_names, video.video_id, debug_graphs)
        html_file.write('</td></tr></table>\n')
        html_file.write('</div>\n')


def _add_segmentation_graphs(html_file: TextIO, data_frame: DataFrame,
                             class_names: List[str], video_id: str,
                             debug_graphs: bool) -> None:
    convert_deltas_to_timestamps(data_frame)
    category_settings = create_custom_category_settings(class_names,
                                                        ColorMapChoice.DARK24)
...
def _write_spotting_html(video: Video, out_video_path: Path, video_dir: Path,
    ...
    with html_path.open("w") as html_file:
        ...
        html_file.write('<div style="overflow-y:auto;height:74%;border:1px '
                        'solid black;">')
        html_file.write('<table width="99%"><tr><td>\n')
        _add_spotting_graphs(html_file, video.spotting_results, class_names,
                             video.video_id, debug_graphs)
        html_file.write('</td></tr></table>\n')
        html_file.write('</div>\n')


def _add_spotting_graphs(html_file: TextIO, data_frame: DataFrame,
                         class_names: List[str], video_id: str,
                         debug_graphs: bool) -> None:
    ...
    if debug_graphs:
        ...
        columns = [
            column for column in SPOTTING_COLUMN_NAMES
            if column in data_frame.columns
        ]
        add_predictions_graphs(html_file, data_frame, columns,
                               category_settings)
    add_click_code(html_file, video_id)
